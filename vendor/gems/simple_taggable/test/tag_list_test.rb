...
class TagListTest < ActiveSupport::TestCase
  ...
  test "#from with a double quoted tag name" do
    assert_equal %w(Fun), TagList.from('"Fun"')
  end
  ...
  test "#from with a double quoted tag name that includes a comma (with comma delimiter)" do
    TagList.delimiter = ','
    assert_equal ["'with", "comma'"], TagList.from("'with, comma'").to_a
  end
  ...
  test "#from with multiple tags with double quotes" do
    assert_equivalent ['A B', 'C'], TagList.from('"A B" C')
  end
